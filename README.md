...
3. **配对并连接**
   ...
   默认情况下，`openclaw relay pair` 会立刻打印配对信息，并把配对窗口保持 5 分钟。你可以手动把 Relay 地址、访问令牌（channel token）和网关公钥填进 `client/index.html`。
   ...
   浏览器打开这个链接后会自动填入三个参数，并立即清掉地址栏中的敏感 fragment。
...
3. **Pair and Connect**
   ...
   By default, `openclaw relay pair` prints the pairing details immediately and keeps the pairing window open for 5 minutes. You can still enter the relay URL, access token, and gateway public key manually in `client/index.html`.
   ...
   Opening that link auto-fills the three pairing values and then clears the sensitive fragment from the browser address bar.
