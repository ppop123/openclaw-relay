...
3. **配对并连接**
   ...
   默认情况下，`openclaw relay pair` 会立刻打印配对信息，并把配对窗口保持 5 分钟。现在推荐的做法是：把终端里打印的 pairing link（`pairing.uri`）粘贴到客户端的 `Pairing link` 输入框。

   如果浏览器客户端有固定地址，也可以直接生成一键连接链接：
...
| `client/` | 浏览器客户端 | 已发布 |
| `desktop/` | Windows/macOS 桌面壳（共享浏览器前端） | 开发中（main） |
| `plugin/` | OpenClaw Gateway 插件 | 已发布 |
| `sdk/js/` | JavaScript SDK | 未实现 |

最新版本：[v0.5.0](https://github.com/ppop123/openclaw-relay/releases/tag/v0.5.0)

桌面壳位于 `desktop/`，面向 Windows 和 macOS，复用同一套 Web client 前端。它目前在 `main` 上开发，下一次正式版本会随主 release 一起发桌面安装包。
...
3. **Pair and Connect**
   ...
   By default, `openclaw relay pair` prints the pairing details immediately and keeps the pairing window open for 5 minutes. The recommended path is to copy the printed pairing link (`pairing.uri`) into the client's `Pairing link` field.

   If your browser client lives at a known URL, you can also generate a one-click handoff link:
...
| `desktop/` | Windows/macOS desktop shell (shared frontend) | In progress on `main` |
| `plugin/` | OpenClaw gateway plugin | Released |
| `sdk/js/` | JavaScript SDK | Not yet implemented |

Latest release: [v0.5.0](https://github.com/ppop123/openclaw-relay/releases/tag/v0.5.0)

The desktop shell lives in `desktop/` for Windows and macOS and reuses the same shared web client frontend. It is being finished on `main` and will ship desktop installers on the next tagged release.
