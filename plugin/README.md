...
- Layer 2 请求/响应/流处理
- 配对状态和已授权客户端持久化
- CLI 命令：`enable`、`pair`、`admin-key`、`clients`、`revoke`、`disable`、`rotate-token`、`status`
...
管理面板中的配置/日志/维护操作需要一个短时有效的 admin key。获取方式：

- 聊天指令：在飞书 / Telegram 等聊天里发送 `/relay_admin`
- CLI：`openclaw relay admin-key`
...
- Officially supported as part of `v0.8.0`
...
- CLI commands: `enable`, `pair`, `admin-key`, `clients`, `revoke`, `disable`, `rotate-token`, `status`
...
### Admin key for management UI

Config/logs/maintenance actions in the web admin UI require a short-lived admin key. Fetch one via:

- Chat command: `/relay_admin` in Feishu / Telegram
- CLI: `openclaw relay admin-key`
