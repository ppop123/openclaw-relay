...
| JavaScript SDK | `sdk/js/` | Node 端 JavaScript/TypeScript SDK（协议层 0–2，仅客户端） |
| 浏览器参考客户端 | `client/` | 基于浏览器的参考客户端 |
| Windows/macOS 桌面壳 | `desktop/` | 围绕共享浏览器前端的 Tauri 桌面壳（Windows/macOS） |
...
| JavaScript SDK | `npm test` 套件 | `vitest` + `tsc` |
| 浏览器参考客户端 | `vitest` 套件 | `vitest` |
| Windows/macOS 桌面壳 | `npm run build:app` | `tauri` + `cargo` |
...
| JS SDK 测试 | `npm test` | JavaScript SDK | 是 |
...
| 桌面壳发布构建 | `.github/workflows/desktop-release.yml`（tag 触发） | Windows/macOS 桌面壳 | 否（tag-only） |
...
| JavaScript SDK | `sdk/js/` | Node-focused JavaScript/TypeScript SDK (protocol layers 0-2, client-side only) |
| Web reference client | `client/` | Browser-based reference client |
| Windows/macOS desktop shell | `desktop/` | Thin Tauri shell around the shared browser client (Windows/macOS) |
...
| JavaScript SDK | `npm test` suite | `vitest` + `tsc` |
| Web reference client | `vitest` suite | `vitest` |
| Windows/macOS desktop shell | `npm run build:app` | `tauri` + `cargo` |
...
| JS SDK tests | `npm test` | JavaScript SDK | Yes |
...
| Desktop release build | `.github/workflows/desktop-release.yml` (tag trigger) | Windows/macOS desktop shell | No (tag-only) |
