...
| Forward secrecy | **No** | Static identity keypairs (generated once at pairing). Compromising an identity private key allows deriving all session keys (past and future). |
...
3. **Key model:** Static X25519 identity keypairs, generated once at pairing time, reused across reconnections. Session key uniqueness comes from a fresh 32-byte random nonce generated by each side per connection, mixed into the HKDF salt. Do NOT generate a new keypair per connection.
...
9. Gateway identity verification follows TOFU — abort connection if public key changes unexpectedly.
